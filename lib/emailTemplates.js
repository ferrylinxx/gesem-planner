...
function responsePage({ reserva, action, token, baseUrl, message, error, googleConnected, justAccepted, changed }) {
  const sessionsHtml = (reserva?.dates || []).map((d, i) => {
    const [y, m, day] = d.split('-');
    const dt = new Date(+y, +m - 1, +day);
    const dl = ['Diumenge', 'Dilluns', 'Dimarts', 'Dimecres', 'Dijous', 'Divendres', 'Dissabte'][dt.getDay()];
  ...
  if (error) {
    ...
    const subscriptionHtml = `
      ...
      <script>
        // Genera l'URL de subscripció via API (passa pel token actual)
        fetch('${baseUrl}/r/${token}/subscribe-url').then(r => r.json()).then(d => {
          if (!d.ok) return;
          const httpsUrl = d.httpsUrl;
          const webcalUrl = d.webcalUrl;
          // URL màgica de Google per a "Afegir per URL" amb pre-rebut del calendari.
          // Funciona amb webcal:// — Google ho detecta i obre el diàleg amb la URL ja
          // omplerta, només cal clicar "Afegir". (Documentat informalment a Stack Overflow
          // i tools com IFTTT/Zapier · més fiable que /settings/addbyurl)
          const googleAddUrl = 'https://www.google.com/calendar/render?cid=' + encodeURIComponent(webcalUrl);
          // URL alternativa per Outlook 365 Web · obre el diàleg de subscripció
          const outlookAddUrl = 'https://outlook.office.com/calendar/addcalendar?url=' + encodeURIComponent(httpsUrl) + '&name=' + encodeURIComponent('GESEM · Formacions');
          document.getElementById('sub-buttons').innerHTML =
            ...
            '<a class="btn" href="' + googleAddUrl + '" target="_blank" rel="noopener" style="background:#fff;border-color:#4285F4;color:#4285F4">📅 Google Calendar</a>' +
            '<a class="btn" href="' + outlookAddUrl + '" target="_blank" rel="noopener" style="background:#fff;border-color:#0078D4;color:#0078D4">📧 Outlook Web</a>';
    ...
    const changeNote = changed ? `
      <div class="card" style="background:#FEF3C7;border-color:#FCD34D">
        <div style="display:flex;align-items:flex-start;gap:10px">
          ...
          <div>
            <div style="font-size:14px;font-weight:600;color:#92400E">Has canviat la teva resposta anterior</div>
            <div style="font-size:12px;color:#A16207;margin-top:3px">Abans havies declinat aquesta reserva i ara l'has acceptada. L'equip de gestió docent rebrà una notificació explicant el canvi.</div>
          </div>
        </div>
      </div>` : '';
    content = `
      ...
      ${subscriptionHtml}
      <div class="card" style="text-align:center;background:#FAFAF7">
        <p style="font-size:12px;color:#71717A;margin-bottom:8px">Has canviat d'opinió?</p>
        <a class="btn" href="${baseUrl}/r/${token}?a=decline" style="color:#DC2626;border-color:#FCA5A5">✕ Canviar a DECLINAR</a>
      </div>`;
  } else if (action === 'decline' && message) {
    const changeNoteDec = changed ? `
      <div class="card" style="background:#FEF3C7;border-color:#FCD34D">
        <div style="display:flex;align-items:flex-start;gap:10px">
          <div style="font-size:22px;line-height:1">⚠️</div>
          <div>
            <div style="font-size:14px;font-weight:600;color:#92400E">Has canviat la teva resposta anterior</div>
            <div style="font-size:12px;color:#A16207;margin-top:3px">Abans havies acceptat aquesta reserva i ara l'has declinada. Si tenies events al teu Google Calendar, s'eliminaran automàticament. L'equip de gestió docent rebrà una notificació.</div>
          </div>
        </div>
      </div>` : '';
    content = `
      ...
      ${changeNoteDec}
      <div class="card" style="text-align:center;background:#FAFAF7">
        <p style="font-size:12px;color:#71717A;margin-bottom:8px">T'has equivocat o has canviat d'opinió?</p>
        <a class="btn primary" href="${baseUrl}/r/${token}?a=accept" style="background:#059669;border-color:#059669">✓ Canviar a ACCEPTAR</a>
      </div>`;
    ...
    content = `
      ...
      <div class="card">
        <h3 style="margin-top:0">${reserva.formadorAccepted == null ? 'Què vols fer?' : 'Vols canviar la teva resposta?'}</h3>
        ${reserva.formadorAccepted !== true ? `<a class="btn primary" href="${acceptUrl}">✓ ${reserva.formadorAccepted === false ? 'Canviar a ACCEPTAR' : 'Acceptar totes les dates'}</a>` : ''}
        ${reserva.formadorAccepted !== false ? `<a class="btn danger" href="${declineUrl}">✕ ${reserva.formadorAccepted === true ? 'Canviar a DECLINAR' : 'Declinar / proposar canvis'}</a>` : ''}
      </div>`;
  }
