...
function responsePage({ reserva, action, token, baseUrl, message, error }) {
  const sessionsHtml = (reserva?.dates || []).map((d, i) => {
    const [y, m, day] = d.split('-');
    const dt = new Date(+y, +m - 1, +day);
    const dl = ['Diumenge', 'Dilluns', 'Dimarts', 'Dimecres', 'Dijous', 'Divendres', 'Dissabte'][dt.getDay()];
  ...
  if (error) {
    ...
    content = `
      ...
      <div class="card">
        <h3 style="margin-top:0">Afegir al calendari</h3>
        <p style="color:#52525B;font-size:14px;margin:8px 0 18px">Tries com vols importar les ${reserva.dates?.length || 0} sessions:</p>
        <a class="btn primary" href="${icsUrl}" download>📎 Descarregar fitxer .ics</a>
        <p style="color:#71717A;font-size:12px;margin-top:12px">Funciona amb Google Calendar, Outlook, Apple Calendar i altres. Obre el fitxer i confirma per afegir totes les sessions de cop.</p>
      </div>`;
