...
function createUser({ email, password, name, role, img, phone, position }) {
  const e = _normEmail(email);
  ...
  const user = {
    ...
    img: img || null,
    phone: phone || '',
    position: position || '',  // càrrec / rol descriptiu
    createdAt: now,
    lastLoginAt: null,
  };
  users.push(user);
...
function updateUser(email, patch) {
  ...
  // img: undefined = no canviar · null = treure · string = nova foto
  if (patch.img !== undefined) {
    if (patch.img === null) delete next.img;
    else next.img = patch.img;
  }
  users[i] = next;
  _saveUsers(users);
...
// ── PRESÈNCIA / ESTAT EN LÍNIA ────────────────────────────────
// `lastActivityAt` s'actualitza amb un heartbeat des del client (cada 60s) +
// també a cada request autenticada que passa pel middleware. Així podem saber
// quins usuaris han estat actius en els últims N minuts.
const ONLINE_THRESHOLD_MS = 3 * 60 * 1000; // 3 min sense activitat = offline
// Debounce de l'escriptura a disc: només persistim si la última activitat
// és més antiga de 15s, per evitar I/O excessiu amb requests freqüents.
const ACTIVITY_PERSIST_DEBOUNCE_MS = 15 * 1000;
...
function updateLastActivity(email) {
  ...
  if (i < 0) return;
  const now = Date.now();
  const prev = users[i].lastActivityAt ? Date.parse(users[i].lastActivityAt) : 0;
  if (now - prev < ACTIVITY_PERSIST_DEBOUNCE_MS) return; // res a fer
  users[i].lastActivityAt = new Date(now).toISOString();
  _saveUsers(users);
}

function isUserOnline(email) {
  const e = _normEmail(email);
  const u = _allUsers().find(x => _normEmail(x.email) === e);
  if (!u || !u.lastActivityAt) return false;
  return (Date.now() - Date.parse(u.lastActivityAt)) < ONLINE_THRESHOLD_MS;
}

// Llista d'emails dels usuaris actualment en línia
function listOnlineUsers() {
  const now = Date.now();
  return _allUsers()
    .filter(u => u.lastActivityAt && (now - Date.parse(u.lastActivityAt)) < ONLINE_THRESHOLD_MS)
    .map(u => u.email);
}
...
const PUBLIC_PATHS = [
  ...
  '/status', '/status.html', '/api/status',
  '/favicon.svg', '/robots.txt', '/.well-known/',
];
const PUBLIC_PREFIXES = [
  '/css/', '/js/', '/img/',
  '/r/',                // tokens públics de formador (HMAC)
  '/cal/',              // webcal subscription feeds (HMAC signed)
...
function middleware() {
  return (req, res, next) => {
    ...
    if (user) {
      req.user = user;
      // Marca activitat (debouncada · escriu a disc cada 15s com a màxim)
      try { updateLastActivity(user.email); } catch(e){}
      return next();
    }
